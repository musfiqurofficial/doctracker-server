...
export async function runSeeding() {
  ...
  // 2. Clear old Doctors & Patients collections
  ...
  for (let i = 1; i <= 500; i++) {
    ...
    const docIdStr = String(assignedDoctor._id);
    doctorPatientCounts[docIdStr] = (doctorPatientCounts[docIdStr] || 0) + 1;
  ...
  // Update doctor patientsCount in MongoDB
  for (const [docId, count] of Object.entries(doctorPatientCounts)) {
    await Doctor.findByIdAndUpdate(docId, { patientsCount: count });
  }

  return {
    adminEmail,
    doctorsCount: createdDoctors.length,
